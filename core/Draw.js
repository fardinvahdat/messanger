...
(function (global) {
  ...
  fabric.util = {
    ...
    getBisector: function (A, B, C) {
      ...
      var ro = fabric.util.calcAngleBetweenVectors(
        fabric.util.rotateVector(AB, alpha),
        AC
      );
    ...
    projectStrokeOnPoints: function (points, options, openPath) {
      ...
      points.forEach(function (p, index) {
        ...
        if (index === 0) {
          C = points[index + 1];
          B = openPath
            ? getStrokeHatVector(fabric.util.createVector(C, A)).addEquals(A)
            : points[points.length - 1];
        } else if (index === points.length - 1) {
          B = points[index - 1];
          C = openPath
            ? getStrokeHatVector(fabric.util.createVector(B, A)).addEquals(A)
            : points[0];
        } else {
          B = points[index - 1];
          C = points[index + 1];
        ...
        if (options.strokeLineJoin === "miter") {
          ...
          if (
            Math.hypot(miterVector.x, miterVector.y) / s <=
    ...
    stylesToArray: function (styles, text) {
      ...
      for (var i = 0; i < textLines.length; i++) {
        ...
        for (var c = 0; c < textLines[i].length; c++) {
          ...
          if (thisStyle) {
            var styleChanged = fabric.util.hasStyleChanged(
              prevStyle,
              thisStyle,
              true
            );
