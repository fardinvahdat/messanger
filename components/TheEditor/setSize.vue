...
<template>
  <div v-if="!mixinState.mSelectMode">
    <!-- <Divider plain orientation="left">{{ 'size' }}</Divider> -->
    <!-- <Form :label-width="40">
      <FormItem :label="'width'" prop="name">
        <InputNumber :max="2000" :min="1" v-model="width" @on-change="setSize"></InputNumber>
      </FormItem>
      <FormItem :label="'height'" prop="name">
        <InputNumber :max="2000" :min="1" v-model="height" @on-change="setSize"></InputNumber>
      </FormItem>
    </Form> -->
    <!-- <Divider plain orientation="left">{{ 'default_size' }}</Divider> -->
