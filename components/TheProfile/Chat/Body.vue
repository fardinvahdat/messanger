<template>
  <section class="grid grid-cols-1 gap-5 px-3 chat-body">
    <template v-for="(chat, index) in chats">
      <template v-if="chat.date">
        <section class="flex justify-center">
          <section class="px-6 py-2 rounded-full" :class="[chat.date == 'Today' ? 'bg-primary-300 bg-opacity-30' : '']">
            <p class="text-gray-500 text-sm font-normal">{{ chat.date }}</p>
          </section>
        </section>
      </template>
      <template v-else-if="chat.text">
        <section class="flex" :class="[chat.isSender ? 'ml-auto' : '']">
          <template v-if="chat.isSender">
            <section class="relative bg-primary-600 px-4 py-2 rounded-lg">
              <p class="text-white text-sm font-normal">{{ chat.text }}</p>
              <section class="flex items-center gap-1 flex-row-reverse mt-1">
                ...
                <p class="text-white text-xs font-normal text-right">{{ chat.time }}</p>
              </section>
            </section>
          </template>
          <template v-else>
            <section class="relative bg-gray-200 px-4 py-2 rounded-lg">
              <section class="absolute left-full top-0">
              </section>
              <p class="text-black text-opacity-80 text-base font-normal">{{ chat.text }}</p>
              <p class="text-black text-opacity-80 text-xs font-normal mt-1 text-right">{{ chat.time }}</p>
            </section>
          </template>
        </section>
      </template>
      <template v-else-if="chat.image">
        <section class="w-9/12 relative" :class="chat.isSender ? 'ml-auto' : ''">
          <img :src="chat.image" class="w-full" @click="openLink(chat.func)"/>
          <section class="absolute right-2 bottom-2">
            <p class="text-white text-xs font-normal mt-1 text-right">{{ chat.time }}</p>
          </section>
        </section>
      </template>
      <template v-else-if="chat.video">
        <section class="relative w-9/12" :class="chat.isSender ? 'ml-auto' : ''">
          <img :src="chat.image" class="w-full" @click="chat.func ? chat.func : ()=> {}"/>
          <section class="absolute flex items-center justify-center w-full h-full">
            <section class="flex grow-0 items-center justify-center w-10 h-10 rounded-full bg-white bg-opacity-50">
    ...
    <section class="flex justify-center fixed bottom-1 left-0 w-full">
      <section
          class="relative add-chat max-w-[414px] bg-white left-auto right-auto w-full z-10 bottom-1 border rounded border-gray-700 flex items-center gap-2 py-2 px-2">
        <input type="file" hidden ref="fileInput" accept="image/*" @change="uploadFile"/>
        ...
        <el-button text class="btn" @click="chooseFile">
...
const chats = ref([
  {date: 'Today'},
  {isSender: true, text: 'Hey Em! Where are you?', time: '11:37 AM'},
  {isSender: false, text: 'I\'m here.', time: '11:37 AM'},
  {isSender: false, image: '/assets/images/profile/map.png', func: 'map', time: '11:37 AM'},
  {isSender: true, text: 'Ok, See you!', time: '11:37 AM'},
  {isSender: false, text: 'Okayy!!', time: '11:37 AM'},
  {date: 'Tues, Apr 11 at 5:34 PM'},
  {isSender: true, image: '/assets/images/profile/baloons.png', time: '11:37 AM'},
  {isSender: true, text: 'Ya, thx, can you send me the video of the Balloons too?', time: '11:37 AM'},
  {isSender: false, text: 'Of course!', time: '11:37 AM'},
  {isSender: false, video: '/assets/images/profile/baloons-video.png', time: '11:37 AM'},
])
const newChat = ref('')
const showEmoji = ref('')
const fileInput = ref('')
...
const openLink = (e) => {
  if (e == 'map') {
    window.open(`https://www.google.com/maps/search/%3Clat%3E,%3Clng%3E/@44.4324336,22.1683007,5z?entry=ttu`, '_blank')
  }
}
const uploadFile = (e) => {
  if (e.target.files) {
