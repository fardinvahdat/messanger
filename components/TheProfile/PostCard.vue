    ...
    <section class="post my-2 flex flex-col gap-2 pb-2 border-b-[1px]">
      <div class="header flex justify-between items-center">
        <div class="left flex gap-2">
          <section
            class="w-[80px] h-[60px] relative flex gap-2 items-center justify-center"
            @click="emit('colMode')"
          >
        ...
        <div class="right relative">
          ...
          <template v-if="isContextMenuVisible">
            <ContextMenu :state="state" :posts="props.posts" @update="emit('update')" />
          </template>
        </div>
      </div>
      <div class="wrapper flex flex-col gap-4">
        ...
        <p class="caption pb-0 text-base">
          ...
          <span
            class="text-blue-600"
            @click="showMoreCaption = !showMoreCaption"
          >
...
import ContextMenu from "@/components/TheProfile/PostCardContextMenu.vue";
...
const emit = defineEmits("colMode", "update");
</script>
