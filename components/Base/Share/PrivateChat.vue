  ...
  >
    <section>
      ...
      <section class="mt-3 grid grid-cols-1 gap-5">
        <section class="flex items-center gap-3" v-for="(user, index) in users">
          <img :src="user.avatar" class="w-12 h-12 rounded-full object-cover border border-gray-300 border-opacity-30" />
      ...
      <section class="mt-2 px-4 py-2 rounded-lg bg-gray-200 flex items-center justify-between gap-3">
        <h2 class="text-sm text-gray-700 font-bold truncate">{{ $route.fullPath }}</h2>
