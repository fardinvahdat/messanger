      ...
      <template v-slot:slide="{ slide }">
        ...
        >
          ...
          <div class="max-w-[414px] fixed w-full px-2 mb-6 bottom-0 chat-page">
            <section class="chat-form bg-white border-0">
...
const slideChangedEvent = (e) => {
  stories.value[stories_component.value.currentStoryIndex].slides[
    e
  ].isVisited = true;
...
const storyList = computed(() => {
  const x = stories.value.filter((item) => item.userId === 3);

  const user = JSON.parse(JSON.stringify(x));
  let slides = [];
  user[0].slides.map((item) => {
    if (item.id == route.params.id) {
      slides.push(item);
    }
  });
  user[0].slides = slides;
  return user;
});
onMounted(() => {
